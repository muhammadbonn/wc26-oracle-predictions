...
def render_upcoming_tab(upcoming_df, pred_dict, conn, username):
    """Renders the Upcoming Matches tab with unique keys to prevent errors."""
    ...
    for round_name, round_group in upcoming_df.groupby('round_name', sort=False):
        # Identify if it's a knockout round
        ...
        with st.expander(round_name, expanded=first_round):
            ...
            for date_str, date_group in round_group.groupby('date_group', sort=False):
                ...
                for _, row in date_group.iterrows():
                    ...
                    # نستخدم الـ key المركب لضمان الفرادة
                    ...
                    with st.expander(f"{home} vs {away} ({m_time.strftime('%H:%M')})", expanded=is_first):
                        ...
                        if predict_goals:
                            ...
                            pred_home = col1.number_input("Home Goals", min_value=0, step=1, value=int(saved_data.get('home_score', 0)) if saved_data else 0, key=f"h_{unique_key}")
                            pred_away = col2.number_input("Away Goals", min_value=0, step=1, value=int(saved_data.get('away_score', 0)) if saved_data else 0, key=f"a_{unique_key}")
                            
                            if is_knockout:
                                col3, col4 = st.columns(2)
                                pred_hp = col3.number_input("Home Pens", min_value=0, step=1, value=int(saved_data.get('home_penalties_score', 0)) if saved_data else 0, key=f"hp_{unique_key}")
                                pred_ap = col4.number_input("Away Pens", min_value=0, step=1, value=int(saved_data.get('away_penalties_score', 0)) if saved_data else 0, key=f"ap_{unique_key}")
