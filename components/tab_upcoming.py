...
def render_upcoming_tab(upcoming_df, pred_dict, conn, username):
    ...
    - Locks predictions for 'TBD' matches.
    - Features dynamic Save/Update buttons.
    - Structured knockout UI.
    """
    ...
    for round_name, round_group in unique_matches.groupby('round_name', sort=False):
        ...
        with st.expander(round_name, expanded=True):
            for _, row in round_group.iterrows():
                ...
                is_tbd = (home == "TBD" or away == "TBD")
                
                with st.expander(f"{home} vs {away}"):
                    if is_tbd:
                        st.warning("Match participants are not decided yet. Predictions will be available soon.")
                    else:
                        ...
                        if is_knockout:
                            ...
                            winner = st.radio("Select Winner:", [home, away], horizontal=True, key=f"win_{unique_key}", 
                                              index=0 if saved_data.get('predicted_outcome') == 'home' else 1 if saved_data.get('predicted_outcome') == 'away' else 0)
